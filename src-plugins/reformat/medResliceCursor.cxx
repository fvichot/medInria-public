...
//----------------------------------------------------------------------------
vtkStandardNewMacro(medResliceCursor);
vtkCxxSetObjectMacro(medResliceCursor, Image, vtkImageData);

//----------------------------------------------------------------------------
medResliceCursor::medResliceCursor()
{
    this->XAxis[0]                = 1.0;
    this->XAxis[1]                = 0.0;
    this->XAxis[2]                = 0.0;
...
//----------------------------------------------------------------------------
void medResliceCursor::BuildCursorTopology()
{
    if (this->Hole)
    {
...
//----------------------------------------------------------------------------
void medResliceCursor::BuildCursorTopologyWithoutHole()
{
    vtkIdType ptIds[2];
...
//----------------------------------------------------------------------------
void medResliceCursor::BuildCursorTopologyWithHole()
{
    vtkIdType ptIds[2];
...
//----------------------------------------------------------------------------
vtkPlane * medResliceCursor::GetPlane( int i )
{
    return this->ReslicePlanes->GetItem(i);
}

//----------------------------------------------------------------------------
vtkPolyData * medResliceCursor::GetPolyData()
{
    this->Update();
...
//----------------------------------------------------------------------------
void medResliceCursor::ComputeAxes()
{
    double normals[3][3];
    for (int i = 0; i < 3; i++)
    {
...
//----------------------------------------------------------------------------
void medResliceCursor::BuildCursorGeometryWithHole()
{
    this->ComputeAxes();
...
//----------------------------------------------------------------------------
void medResliceCursor::BuildCursorGeometryWithoutHole()
{
    this->ComputeAxes();
...
//----------------------------------------------------------------------------
void medResliceCursor::BuildCursorGeometry()
{
    if (this->Hole)
    {
...
//----------------------------------------------------------------------------
void medResliceCursor::BuildPolyData()
{
    this->ComputeAxes();
    ...
    if (1)
    {
        ...
        double pts[30][3];
        double spacing; // lets use ht for the time being
        double nbSlices;
        double ptsExtent[30][3];
        // TODO : determiner les orientations surlesquelles cela doit etre fait.
        // Determiner spacing
        // Determiner le nombre de slice
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            if (1)
            {
                ptsExtent[2][i] = pts[0][i] -2* ht[1] * this->YAxis[i] -2* ht[2] * this->ZAxis[i];
                ptsExtent[3][i] = pts[1][i] - 2* ht[1] * this->YAxis[i] - 2* ht[2] * this->ZAxis[i];
                ptsExtent[4][i] = pts[0][i] + 2* ht[1] * this->YAxis[i] - 2* ht[2] * this->ZAxis[i];
                ptsExtent[5][i] = pts[1][i] + 2* ht[1] * this->YAxis[i] - 2* ht[2] * this->ZAxis[i];
                ptsExtent[6][i] = pts[0][i] + 2* ht[1] * this->YAxis[i] + 2* ht[2] * this->ZAxis[i];
                ptsExtent[7][i] = pts[1][i] + 2* ht[1] * this->YAxis[i] + 2* ht[2] * this->ZAxis[i];
                ptsExtent[8][i] = pts[0][i] - 2* ht[1] * this->YAxis[i] + 2* ht[2] * this->ZAxis[i];
                ptsExtent[9][i] = pts[1][i] - 2* ht[1] * this->YAxis[i] + 2* ht[2] * this->ZAxis[i];
            }
        }
...
//----------------------------------------------------------------------------
void medResliceCursor::SetCenter( double _arg1, double _arg2, double _arg3 )
{
...
//----------------------------------------------------------------------------
void medResliceCursor::SetCenter(double _arg[3])
{
    this->SetCenter(_arg[0], _arg[1], _arg[2]);
}

//----------------------------------------------------------------------------
vtkPolyData * medResliceCursor::GetCenterlineAxisPolyData( int axis )
{
    this->Update();
...
//----------------------------------------------------------------------------
double * medResliceCursor::GetAxis( int i )
{
    if (i == 0)
    {
        return this->XAxis;
...
//----------------------------------------------------------------------------
unsigned long int medResliceCursor::GetMTime()
{
...
//----------------------------------------------------------------------------
void medResliceCursor::PrintSelf(ostream& os, vtkIndent indent)
{
