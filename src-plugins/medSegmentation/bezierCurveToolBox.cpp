...
*/
    ...
    qDebug() << " stats.std : " << stats.std;
    qDebug() << " stats.mean : " << stats.mean;
...
double* bezierCurveToolBox::data()
{
    double out[2];
    out[0] = stat.mean;
    out[1] = stat.std;
...
void bezierCurveToolBox::computeStatistics()
{
    ...
    QList<QPair<vtkPolygon*,PlaneIndexSlicePair> > listPolygon = createImagePolygons(listPolyData);
    stat = ComputeHistogram(listPolygon[0]);
    polyRoi->setRoiStatistics(stat);
    histogramToolBox->setStatistics(stat.area,stat.perimeter,stat.mean,stat.std,stat.sum,stat.min,stat.max);
}
